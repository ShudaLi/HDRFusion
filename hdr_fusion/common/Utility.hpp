...
SE3Group<T> extractRTFromBuffer(const T* aHostTmp_)
{
	...
	for (int r = 0; r < 6; ++r){   // rows
		for (int c = r; c < 7; ++c) { // cols + b
			T value = aHostTmp_[sShift++];
			if (c == 6)       // vector b
				b.data()[r] = value;
			else
				A.data()[c * 6 + r] = A.data()[r * 6 + c] = value;
		}//for each col
	}//for each row
	...
	//Eigen::Matrix<T, 6, 1> b;
	////retrieve A and b from cvmSumBuf
	//A(0, 0) = aHostTmp_[0]; A(0, 1) = aHostTmp_[1]; A(0, 2) = aHostTmp_[2]; A(0, 3) = aHostTmp_[3];  A(0, 4) = aHostTmp_[4];  A(0, 5) = aHostTmp_[5]; b(0) = aHostTmp_[6];
	//A(1, 0) = aHostTmp_[1]; A(1, 1) = aHostTmp_[7]; A(1, 2) = aHostTmp_[8]; A(1, 3) = aHostTmp_[9];  A(1, 4) = aHostTmp_[10]; A(1, 5) = aHostTmp_[11]; b(1) = aHostTmp_[12];
	//A(2, 0) = aHostTmp_[2]; A(2, 1) = aHostTmp_[8]; A(2, 2) = aHostTmp_[13]; A(2, 3) = aHostTmp_[14];  A(2, 4) = aHostTmp_[15]; A(2, 5) = aHostTmp_[16]; b(2) = aHostTmp_[17];
	//A(3, 0) = aHostTmp_[3]; A(3, 1) = aHostTmp_[9]; A(3, 2) = aHostTmp_[14]; A(3, 3) = aHostTmp_[18];  A(3, 4) = aHostTmp_[19]; A(3, 5) = aHostTmp_[20]; b(3) = aHostTmp_[21];
	//A(4, 0) = aHostTmp_[4]; A(4, 1) = aHostTmp_[10]; A(4, 2) = aHostTmp_[15]; A(4, 3) = aHostTmp_[19];  A(4, 4) = aHostTmp_[22]; A(4, 5) = aHostTmp_[23]; b(4) = aHostTmp_[24];
	//A(5, 0) = aHostTmp_[5]; A(5, 1) = aHostTmp_[11]; A(5, 2) = aHostTmp_[16]; A(5, 3) = aHostTmp_[20];  A(5, 4) = aHostTmp_[23]; A(5, 5) = aHostTmp_[25]; b(5) = aHostTmp_[26];
	...
	//cout << SO3Group<T>::exp(result.head<3>()).matrix() << endl;
	//cout << SO3Group<T>::exp(result.tail<3>()).matrix() << endl;
...
template<typename T>
SE3Group<T> extractRTFromBuffer2(const T* aHostTmp_)
{
	Eigen::Matrix<T, 6, 6> A;
	Eigen::Matrix<T, 6, 1> b;
	//retrieve A and b from cvmSumBuf
	...
	Eigen::Matrix<T, 6, 1> result = A.llt().solve(b);
	//cout << result << endl;
	SE3Group<T> T_rl(SO3Group<T>::exp(SE3Group<T>::Point(result(3), result(4), result(5))),
					 SE3Group<T>::Point(result(0), result(1), result(2)));
	return T_rl;
}

template<typename T>
SO3Group<T> extractRFromBuffer(const T* aHostTmp_)
{
	//declare A and b
	Eigen::Matrix<T, 3, 3> A;
	Eigen::Matrix<T, 3, 1> b;
	//retrieve A and b from cvmSumBuf
	A(0, 0) = aHostTmp_[0]; A(0, 1) = aHostTmp_[1]; A(0, 2) = aHostTmp_[2]; b(0) = aHostTmp_[3];
	A(1, 0) = aHostTmp_[1]; A(1, 1) = aHostTmp_[4]; A(1, 2) = aHostTmp_[5]; b(1) = aHostTmp_[6];
	A(2, 0) = aHostTmp_[2]; A(2, 1) = aHostTmp_[5]; A(2, 2) = aHostTmp_[7]; b(2) = aHostTmp_[8];
	//cout << A << endl;
	//cout << b << endl;
	//checking nullspace
	T dDet = A.determinant();
	...
	Eigen::Matrix<T, 3, 1> result = A.llt().solve(b);

	return SO3Group<T>::exp(result);
}

template<typename T>
Eigen::Matrix<T, 2, 1> extractABFromBuffer(const T* aHostTmp_)
{
	//declare A and b
	...
	//retrieve A and b from cvmSumBuf
	A(0, 0) = aHostTmp_[0]; A(0, 1) = aHostTmp_[1]; b(0) = aHostTmp_[2];
	A(1, 0) = aHostTmp_[1]; A(1, 1) = aHostTmp_[3]; b(1) = aHostTmp_[4];
	//cout << A << endl;
	//cout << b << endl;
	//checking nullspace
	T dDet = A.determinant();
	if (fabs(dDet) < 1e-15 || dDet != dDet){
		if (dDet != dDet)
			PRINTSTR("Failure -- dDet cannot be qnan. ");
		return Eigen::Matrix<T, 2, 1>(0, 0);
	}//if dDet is rational

	Eigen::Matrix<T, 2, 1> result = A.llt().solve(b);
